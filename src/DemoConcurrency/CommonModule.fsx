...
module TimeMeasurement =
    ...
    type OperationTimer(label) =
        ...
        let collectionCount = System.GC.CollectionCount(0)
        ...
        interface System.IDisposable with
            member x.Dispose() = 
                let result = System.String.Format("{0,7:N0} (GCs={1,3}) {2}", 
                                sw.Elapsed.TotalMilliseconds, System.GC.CollectionCount(0) - collectionCount, label)
                printfn "%s" result
...
module StartPhotoViewr = 
    let start() = System.Diagnostics.Process.Start(@"C:\Demo\ConcurrecnyFSharp\DemoConcurrency_V3\PhotoViewer\bin\Debug\PhotoViewer.exe")