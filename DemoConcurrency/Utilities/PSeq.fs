...
module PSeq = 
    ...
    let inline fold<'T, 'State> (f : 'State -> 'T -> 'State) acc s = 
        ParallelEnumerable.Aggregate(toP(s),acc,Func<_,_,_>(f))

    let inline reduce f s = 
        ParallelEnumerable.Aggregate(toP(s), Func<_,_,_>(f))

    let inline mapReduce initf mapf reducef selectf (input:pseq<_>) = 
        input.Aggregate<'TSource, 'TAccumulate, 'TResult>
            ( new Func<'TAccumulate>(initf), new Func<'TAccumulate, 'TSource, 'TAccumulate>(mapf), 
              new Func<'TAccumulate, 'TAccumulate, 'TAccumulate>(reducef), new Func<'TAccumulate, 'TResult>(selectf))

    let inline exists p s = 
        ParallelEnumerable.Any(toP(s), Func<_,_>(p))

    let inline forall p s = 
        ParallelEnumerable.All(toP(s), Func<_,_>(p))

    let inline exists2 (f : 'T -> 'U -> bool) s1 s2 = 
        ParallelEnumerable.Any(ParallelEnumerable.Zip(toP(s1), toP(s2),Func<_,_,_>(f)), Func<_,_>(id))

    let inline forall2 (f : 'T -> 'U -> bool) s1 s2 = 
        ParallelEnumerable.All(ParallelEnumerable.Zip(toP(s1), toP(s2),Func<_,_,_>(f)), Func<_,_>(id))

    let inline filter p s = 
